...
class Batalha():
    ...
    def atacar(self):
        ...
        input(f"Você deu {playerdano} de dano!")
        return playerdano
    ...
    def fugir(self):
        show_mapa(self.jogador.show_map_of_player(), True, colorama['LIGHTGREEN_EX']+"Você tenta fugir."+colorama['RESET'])
    ...
    def chose_option(self):
        show_mapa(self.jogador.show_map_of_player(), True, f"{colorama['LIGHTRED_EX']}{self.inimigo.nome} te ataca!{colorama['RESET']}\n{colorama['LIGHTBLUE_EX']}Oque você faz?{colorama['RESET']}")
        while True:
            msg = ""
            ...
            print(colorama['RED'])
            print("#"*self.jogador.vida, end=colorama['LIGHTBLACK_EX'])
            print("#"*(self.jogador.vida_max-self.jogador.vida), end=colorama['RESET'])
            print()
            ...
            print(colorama['RED'])
            print("#"*self.inimigo.vida, end=colorama['LIGHTBLACK_EX'])
            print("#"*(self.inimigo.vida_max-self.inimigo.vida), end=colorama['RESET'])
            print()
            print()
            print(" "*25,end="")
            ...
            cor = colorama['CYAN']
            reset = colorama['RESET']

            if choice == 1:
                msg = cor+f"Você ataca!{reset}\n"
                msg += cor+f"Você deu: {self.atacar()} de dano{reset}"
            if choice == 2:
                _ = self.item()
                if _ == 0:
                    atack = False
                ...
                _ = self.fugir()
                if _ == 2:
                    msg = f"{colorama['LIGHTGREEN_EX']}Você consegue escapar!{colorama['RESET']}"
    ...
    for numero1, valor_y in enumerate(mapa_cenario):
        ...
        for numero2, valor_x in enumerate(valor_y):
            ...
            if valor_x in list(cenario_objs_letra.keys()):
                ...
                if letra_anterior == " ":
                    if type(letra) == type(""):
                        sombra = colorama['LIGHTBLACK_EX']+letra+colorama['RESET']
                    if type(letra) == type([]):
                        sombra = []
                        for let in letra:
                            sombra.append(colorama['LIGHTBLACK_EX']+let+colorama['RESET'])
                ...
                linha_x += colorama['MAGENTA']+"?"+colorama['BLACK']+("?"*((amplificacao+int(amplificacao*1.1))-1))
        ...
        atributo_lado = [
            [],
            [colorama['CYAN'], "Nome: ",personagem.nome, colorama['RESET']],
            [colorama['MAGENTA'], "Classe: ",personagem.classe, colorama['RESET']],
            [colorama['YELLOW'], "Nível: ",personagem.nivel, colorama['RESET']],
            [colorama['RED'], "HP: ",personagem.vida, colorama['RESET']],
            [colorama['LIGHTMAGENTA_EX'], "Defesa: ", personagem.defesa, colorama['RESET']],
            [],
            ["Inventory:"],
            [],
            ["Item Equipado:"],
